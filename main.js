        ...
        const addAnItemVersion = async (event, key, itemVersion) => {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    let response = await dbGet(db, `SELECT * FROM itemVersions WHERE id="${itemVersion.id}" AND version=${itemVersion.version}`)
                    if (response.status === "ok" && !response.row) {
                        let command = prepareCommand();
                        let response = await dbRun(db, command);
                        /*response = await dbGet(db, "SELECT * FROM itemVersions");
                        if (response.status === 'ok' && response.row) {
                            let result = parseItemVersion(response.row);
                            compareItemVersion(itemVersion, result.itemVersion);
                            resolve({status:"ok"});
                        } else {
